...
customText,subTitle,imageId,bigImage }) => {
    ...
    >
            <div className={bigImage ? 'image-text-box-big' : 'image-text-box'}
            >
                {rev ? (
   ...
   {link != null && 
       <Link to={link}>
           <button className="button">{linkText}</button>
                ...
                <div className={bigImage ? 'image-box-big' : 'image-box'}>
                    <h1 className="title-text">{title}</h1>
                    <motion.img
                        id={imageId}
                        src={image}
                        loading="lazy"
                        className={bigImage ? 'content-image-big' : 'content-image'}
