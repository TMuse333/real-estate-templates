...
const Navbar3 = () => {
  ...
  const [returnClicked, setReturnCLicked] = useState(true)
...
setReturnCLicked(false)   
  ...
  const secondaryNavStyle = (index) => {
    const selected = activeMenuItem === index && activeMenuItem != null && !returnClicked;

    return {
  
        right: selected ? '0%' : returnClicked ? '-200%' : '-200%',
        transition: 'right 0.3s ease-in',
        paddingTop: '4rem',
        paddingLeft: '2rem',
        // marginRight:'3.8rem',
