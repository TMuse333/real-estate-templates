...
const Draggable = () => {
    ...
    const handleMouseMove = (e) => {
        if (isDragging ) {
          ...
          setTotalScroll((prevTotalScroll) => {
            ...
            const newTotalScroll = Math.min(Math.max(prevTotalScroll + limitedMagnitude, -50), 100);
          ...
          setObjectPosition((prevObjectPosition) => {
            ...
            const cappedY = Math.min(Math.max(newY, -50), 100);
      
            // Check if the new position is equal to one of the limits
            if (cappedX === 0 || cappedX === 100 || cappedY === -50 || cappedY === 100) {
  ...
  return (
    <div className="draggable-container">
      ...
      />
      <p>Total Scroll: {objectPosition.y}</p>
    </div>
  );
};
