...
const Draggable = () => {
  ...
  const threshold = 5; // Adjust this threshold value
    ...
    const handleMouseMove = (e) => {
      if (isDragging) {
        ...
        setObjectPosition((prevObjectPosition) => {
          ...
          const cappedX = Math.min(Math.max(newX, threshold), 100 - threshold);
          const cappedY = Math.min(Math.max(newY, threshold), 100 - threshold);

          if (
            cappedX === threshold ||
            cappedX === 100 - threshold ||
            cappedY === threshold ||
            cappedY === 100 - threshold
          ) {
