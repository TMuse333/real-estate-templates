...
const FullScreenSlide = ({ video, image, id }) => {
  ...
  useEffect(() => {
    const handleWheel = (event) => {
      ...
      // console.log('magnitude of scroll',scrollMagnitude)
      // console.log('direction of scroll:',scrollDirection)

      setScrollPower((prevScrollPower) => {
        // If scrolling up, use the regular scrollMagnitude
        // If scrolling down and text position is in the range of 50-90, add a 1.5 multiplier to scrollMagnitude
        return scrollDirection === 'up' ? -scrollMagnitude : (newTextPosition >= 50 && newTextPosition <= 90) ? 1.5 * scrollMagnitude : scrollMagnitude;
      });
