...
const FullScreenSlide = ({ video, image, id }) => {
  ...
  useEffect(() => {
    const handleWheel = (event) => {
      ...
      const scrollPower = scrollDirection === 'up' ? -scrollMagnitude : scrollMagnitude;

      // Update the ref with the latest scrollPower
      scrollPowerRef.current = scrollPower;
  ...
  useEffect(() => {
    ...
    const handleScroll = (event) => {
      ...
      const scrollMagnitude = Math.abs(deltaY) / 9;

      // Calculate scrollPower based on scrollDirection and scrollMagnitude
      const scrollPower = scrollDirection === 'up' ? -scrollMagnitude : scrollMagnitude;
      ...
      if ((elementTop - windowHeight / 2) + 225 <= 0) {
        setTextPosition((prevTextPosition) => {
          // Update the text position based on the latest scrollPower
          const newTextPosition = prevTextPosition + scrollPowerRef.current;

          // Log the adjustment to the text position
          console.log('Adjustment to Text Position:', scrollPowerRef.current);
  ...
  }, [id, setTextPosition, setBottomReached]);
  ...
  return (
    <div className="full-slide-container" style={style}>
      {video && (
        <>
          <video
            ref={videoRef}
